<template>
  <div class="space-y-6">
    ...
    <p v-if="dashboard.error" class="text-sm text-red-600">{{ dashboard.error.messages?.[0] || 'Failed to load' }}</p>

    <div v-if="dashboard.loading && !dashboard.data" class="text-sm text-gray-500 dark:text-gray-400">Loading…</div>
    ...
    <template v-else-if="dashboard.data">
      <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
        ...
        <KpiCard
          ...
          :to="{ name: 'Reservations', query: { status: 'checked_in' } }"
        />
        <KpiCard label="Housekeeping Open" icon="🧹" :value="dashboard.data.pending_housekeeping_tasks" />
        <KpiCard label="Maintenance Open" icon="🛠️" :value="dashboard.data.open_maintenance_requests" />
      </div>

      <div class="rounded-lg border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
        <div class="mb-4 flex items-center justify-between">
          <h2 class="text-sm font-semibold text-gray-900 dark:text-gray-100">Rooms by Status</h2>
        ...
        <div v-if="!Object.keys(dashboard.data.rooms_by_status).length" class="text-sm text-gray-500">
          No rooms found for this property yet.
        </div>
