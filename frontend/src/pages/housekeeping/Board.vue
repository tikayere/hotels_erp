<template>
  <div class="space-y-4">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h1 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Housekeeping</h1>
      <div class="flex flex-wrap items-center gap-2">
        ...
        <Button variant="solid" @click="openNewTask">+ New Task</Button>
      </div>
    </div>

    <p v-if="tasks.error" class="text-sm text-red-600">{{ tasks.error.messages?.[0] || 'Failed to load' }}</p>
    <div v-if="tasks.loading && !tasks.data" class="text-sm text-gray-500 dark:text-gray-400">Loading…</div>
    <div v-else-if="!tasks.data?.length" class="text-sm text-gray-500 dark:text-gray-400">No tasks match these filters.</div>
    ...
    <div v-else class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
      ...
      >
        <div class="flex items-start justify-between">
          <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">Room {{ t.room_number || t.room }}</span>
          <StatusBadge :status="t.status" />
        </div>
        <div class="text-xs text-gray-500 dark:text-gray-400">{{ statusLabel(t.type) }} · Floor {{ t.floor || '—' }}</div>
        <div class="text-xs text-gray-600 dark:text-gray-300">
          Due {{ formatDateTime(t.due_at) }}
        </div>
        <div class="text-xs text-gray-400">{{ t.assigned_to || 'Unassigned' }}</div>
      </RouterLink>
    </div>

    <Dialog v-model="newTaskOpen" :options="{ title: 'New Housekeeping Task' }">
      <template #body-content>
        ...
        <div class="mt-4 flex justify-end gap-2">
          <Button variant="outline" @click="newTaskOpen = false">Cancel</Button>
