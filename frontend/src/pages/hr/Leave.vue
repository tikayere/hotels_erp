<template>
  <div class="space-y-4">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h1 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Leave Applications</h1>
      <div class="flex flex-wrap items-center gap-2">
        <select v-model="statusFilter" class="select-field">
          <option value="">All statuses</option>
          <option v-for="s in STATUSES" :key="s" :value="s">{{ statusLabel(s) }}</option>
        </select>
        <Button variant="solid" @click="openNew">+ New Application</Button>
      </div>
    </div>

    <p v-if="leaves.error" class="text-sm text-red-600">{{ leaves.error.messages?.[0] || 'Failed to load' }}</p>
      ...
      <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-800">
        ...
        <tbody class="divide-y divide-gray-50 text-sm dark:divide-gray-800/60">
          ...
          <tr v-else-if="!leaves.data?.length">
            <td class="px-4 py-6 text-gray-500 dark:text-gray-400" colspan="5">No applications match these filters.</td>
          </tr>
          <tr v-for="l in leaves.data" :key="l.name">
            ...
            <td class="px-4 py-3 text-right">
              <div v-if="l.status === 'pending'" class="flex justify-end gap-2">
                <Button size="sm" variant="outline" theme="green" @click="doApprove(l.name)">Approve</Button>
                <Button size="sm" variant="outline" theme="red" @click="doReject(l.name)">Reject</Button>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <Dialog v-model="newOpen" :options="{ title: 'New Leave Application' }">
      <template #body-content>
        <div class="space-y-3">
          <Field label="Staff docname">
            <input v-model="form.staff" type="text" placeholder="e.g. HR-STAFF-0001" class="input-field" />
          </Field>
