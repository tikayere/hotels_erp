<template>
  <div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h1 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Finance</h1>
      <Button variant="solid" @click="openNewTxn">+ New Transaction</Button>
    </div>
    ...
    <p v-if="txns.error" class="text-sm text-red-600">{{ txns.error.messages?.[0] || 'Failed to load' }}</p>
      ...
      <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-800">
        ...
        <tbody class="divide-y divide-gray-50 text-sm dark:divide-gray-800/60">
          ...
          <tr v-else-if="!rows.length">
            <td class="px-4 py-6 text-gray-500 dark:text-gray-400" colspan="6">No transactions match these filters.</td>
          </tr>
          <tr v-for="t in rows" :key="t.name">
            ...
            <td class="px-4 py-3 text-right">
              <Button v-if="t.docstatus === 0" size="sm" variant="outline" @click="doSubmit(t.name)">Submit</Button>
    ...
    <div class="flex items-center justify-between text-sm text-gray-500 dark:text-gray-400">
      <span>{{ txns.data?.total_count ?? 0 }} transactions</span>
    ...
    <Dialog v-model="newTxnOpen" :options="{ title: 'New Finance Transaction' }">
      <template #body-content>
        ...
        <div class="mt-4 flex justify-end gap-2">
          <Button variant="outline" @click="newTxnOpen = false">Cancel</Button>
          <Button
            variant="solid"
...
const TYPES = ['revenue', 'refund', 'tax', 'expense']
