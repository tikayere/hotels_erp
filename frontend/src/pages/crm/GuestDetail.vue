<template>
  <div class="max-w-2xl space-y-6">
    ...
    <p v-if="guest.error" class="text-sm text-red-600">{{ guest.error.messages?.[0] || 'Failed to load this guest' }}</p>
    <div v-if="guest.loading && !guest.data" class="text-sm text-gray-500 dark:text-gray-400">Loading…</div>
    ...
    <template v-else-if="guest.data">
      <div class="flex items-start justify-between rounded-lg border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
        ...
        <span v-if="guest.data.vip_status" class="text-sm font-medium text-amber-600 dark:text-amber-400">★ VIP</span>
      </div>

      <Card title="Communications">
        <template #actions>
          <Button size="sm" variant="outline" @click="openLogComm">+ Log</Button>
        </template>
        <ul v-if="guest.data.communications?.length" class="divide-y divide-gray-50 text-sm dark:divide-gray-800/60">
          <li v-for="c in guest.data.communications" :key="c.name" class="py-2">
            ...
            <div class="text-xs text-gray-500 dark:text-gray-400">{{ c.direction }} · {{ c.channel }}</div>
            <p v-if="c.message" class="mt-1 text-gray-700 dark:text-gray-300">{{ c.message }}</p>
          </li>
        </ul>
        <p v-else class="text-sm text-gray-500 dark:text-gray-400">No communications logged yet.</p>
      </Card>

      <Card title="Complaints">
        <template #actions>
          <Button size="sm" variant="outline" @click="openNewComplaint">+ New</Button>
        </template>
        <ul v-if="guest.data.complaints?.length" class="divide-y divide-gray-50 text-sm dark:divide-gray-800/60">
          <li v-for="c in guest.data.complaints" :key="c.name" class="py-2">
            <div class="flex items-center justify-between">
              <span class="font-medium text-gray-900 dark:text-gray-100">{{ statusLabel(c.category) || 'Complaint' }}</span>
              <StatusBadge :status="c.status" />
            </div>
            <p class="mt-1 text-gray-700 dark:text-gray-300">{{ c.description }}</p>
          </li>
        </ul>
        <p v-else class="text-sm text-gray-500 dark:text-gray-400">No complaints on file.</p>
      </Card>
    </template>

    <Dialog v-model="commOpen" :options="{ title: 'Log Communication' }">
      <template #body-content>
        ...
        <div class="mt-4 flex justify-end gap-2">
          <Button variant="outline" @click="commOpen = false">Cancel</Button>
          <Button variant="solid" :loading="createComm.loading" @click="submitComm">Log</Button>
        </div>
      </template>
    </Dialog>

    <Dialog v-model="complaintOpen" :options="{ title: 'New Complaint' }">
      <template #body-content>
        <div class="space-y-3">
        ...
        <div class="mt-4 flex justify-end gap-2">
          <Button variant="outline" @click="complaintOpen = false">Cancel</Button>
...
const complaintForm = reactive({ category: 'room', reservation: '', description: '' })
function openNewComplaint() {
  Object.assign(complaintForm, { category: 'room', reservation: '', description: '' })
  createComplaint.error = null
  complaintOpen.value = true
}
function submitComplaint() {
  createComplaint.submit(
    { guest: props.id, category: complaintForm.category, description: complaintForm.description, reservation: complaintForm.reservation || undefined },
